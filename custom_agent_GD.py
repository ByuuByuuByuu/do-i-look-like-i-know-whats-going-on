...
from custom_model_GD import QModel, QTrainer
...
class Agent:
    ...
    def __init__(self):
        ...
        self.trainer = QTrainer(alpha = self.alpha, model = self.model, gamma = self.gamma)
    ...
    def train_long_memory(self, W1, b1, W2, b2, counter, sample_size_counter):
        ...
        return self.trainer.train_step(W1, b1, W2, b2, states, actions, rewards, next_states, dones, counter, sample_size_counter)



    def train_short_memory(self, W1, b1, W2, b2, state, action, reward, next_state, done, counter, sample_size_counter):
        return self.trainer.train_step(W1, b1, W2, b2, state, action, reward, next_state, done, counter, sample_size_counter)
...
def train():
    ...
    while True:
        ...
        W1, b1, W2, b2 = agent.train_short_memory(W1, b1, W2, b2, current_state, selected_move, reward, new_state, done, counter, sample_size_counter)
        ...
        if done:
            ...
            game.reset()
            agent.n_games += 1
            W1, b1, W2, b2 = agent.train_long_memory(W1, b1, W2, b2, counter, sample_size_counter)
            # agent.train_long_memory(W1, b1, W2, b2)

            if score > record:
                record = score
                agent.model.save(W1,b1,W2,b2)
